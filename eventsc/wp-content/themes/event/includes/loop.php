		...
		<?php if($themify->hide_meta != 'yes'): ?>
			<div class='post-category-wrap'>
				<?php if($themify->hide_meta_category != 'yes'): ?>
					<?php the_terms( get_the_ID(), 'post' != get_post_type()? get_post_type() . '-category': 'category', ' <span class="post-category">', ', ', '<i class="divider-small"></i></span>' );	?>
				<?php endif; ?>
		...
		<?php if ( $themify->hide_meta != 'yes' || $themify->hide_date != 'yes' ) : ?>
			<div class="post-meta entry-meta clearfix">
				<div class="meta-left clearfix">
					...
					<?php endif; ?>
					<?php if ( $themify->hide_date != 'yes' && is_single() ): ?>
						<time datetime="<?php the_time('o-m-d') ?>" class="post-date entry-date updated" itemprop="datePublished"><?php the_time(apply_filters('themify_loop_date', 'M j, Y')) ?></time>
					<?php endif; //post date ?>
				</div>
