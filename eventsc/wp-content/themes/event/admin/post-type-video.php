	...
	class Themify_Video {
		...
		function fetch_video_image( $make_tag = true, $width = '' ) {
			...
			if ( preg_match( '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $video_url, $match ) ) {
				...
				$response_body = wp_remote_retrieve_body( wp_remote_get( 'http://vimeo.com/api/oembed.json?url='.urlencode( $video_url ) ) );
				if ( '' != $response_body ) {
					$vimeo = json_decode( $response_body );
					$image_url = $vimeo->thumbnail_url;
				}
			} elseif( false !== stripos( $video_url, 'funnyordie' ) ) {
				$response_body = wp_remote_retrieve_body( wp_remote_get( 'http://www.funnyordie.com/oembed.json?url='.urlencode( $video_url ) ) );
				if ( '' != $response_body ) {
			...
			if ( $make_tag ) {
				return '<img src="' . themify_https_esc( $image_url ) . '" width="' . $themify->width . '" />';
			}
			return $image_url;
		}
