					...
					if ( 'yes' == $event_tab && ! $is_slider ) {
						...
						if ( 0 == $tab ) {
							if ( ! empty( $events['upcoming'] ) ) {
								$htabs .= '<a href="#" class="htab-link" data-tab="upcoming">' . __( 'Ahora y Próximos ', 'themify' );
								$btabs .= '<li class="upcoming-events btab-panel">';
							} elseif ( ! empty( $events['past'] ) ) {
								$htabs .= '<a href="#" class="htab-link" data-tab="past">' . __( 'Pasados', 'themify' );
	...
	function widget( $args, $instance ) {
		...
		foreach( $events as $posts ) {
			if ( $posts ) {
				...
				$themify->hide_meta = 'yes' == $meta? 'no': 'yes';
				...
				$btabs .= '
					<div class="shortcode ' . $post_type  . '-widget ' . $style . ' '. $themify->post_layout .'">' .
